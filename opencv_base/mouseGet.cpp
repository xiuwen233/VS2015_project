...
Mat mySRCimg, myDSTimg, g_grayImage, g_maskImage;//定义原始图、目标图、灰度图、掩模图  
...
static void onMouse(int event, int x, int y, int, void*)
{
	...
	Mat dst = g_bIsColor ? myDSTimg : g_grayImage;//目标图的赋值  
	int area;
...
int mouseOpreat()
{
	...
	if (!mySRCimg.data) { printf("Oh，no，读取图片image0错误~！ \n"); return false; }

	//显示帮助文字  
	ShowHelpText();

	mySRCimg.copyTo(myDSTimg);//拷贝源图到目标图  
	cvtColor(mySRCimg, g_grayImage, COLOR_BGR2GRAY);//转换三通道的image0到灰度图  
	g_maskImage.create(mySRCimg.rows + 2, mySRCimg.cols + 2, CV_8UC1);//利用image0的尺寸来初始化掩膜mask  
	...
	{
		//先显示效果图  
		imshow("效果图", g_bIsColor ? myDSTimg : g_grayImage);
